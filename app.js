(() => {
  ...
  function draw() {
    ...
    for (let r = 0; r < BOARD_SIZE; r++) {
      for (let c = 0; c < BOARD_SIZE; c++) {
        ...
        const x = padding + r * cell;
        const y = padding + c * cell;
        const radius = cell * 0.35;

        let gradient;
        if (color === WHITE) {
          gradient = ctx.createRadialGradient(x - radius / 2, y - radius / 2, 2, x, y, radius);
    ...
    if (lastMove) {
      const x = padding + lastMove.row * cell;
      const y = padding + lastMove.col * cell;
    ...
    if (gameResult === DRAW) {
      ...
      drawOverlay('Black wins');
    } else if (gameResult === WHITE) {
      drawOverlay('White wins');
    }
  }
  ...
  function canvasToCell(x, y) {
    ...
    const fx = (x - padding) / cell;
    const fy = (y - padding) / cell;
    const row = Math.round(fx);
    const col = Math.round(fy);

    if (row < 0 || row >= BOARD_SIZE || col < 0 || col >= BOARD_SIZE) return null;

    const gridX = padding + row * cell;
    const gridY = padding + col * cell;
    const dx = x - gridX;
    const dy = y - gridY;
